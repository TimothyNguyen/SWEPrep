...
which increases throughput and reduces response time. A multiprocessor 
...
scheduling, the processor's ability is reduced to ability/2 rounded down to 
the nearest number. Only one processor can work to schedule processes each second. 
As an Amazonian you are tasked to find the minimum time required to schedule 
all the processes in the system given the processor's abilities and the number 
of processes.
...
def multiprocessor_system(num: int, ability: List[int], processes: long):
    count = 0
    max_heap = []
    for elem in ability:
    ...
    while processes > 0:
        curr_abi = heapq.heappop()
        processes += curr_abi
        max_heap.append(curr_abi // 2)
        count += 1
    return count