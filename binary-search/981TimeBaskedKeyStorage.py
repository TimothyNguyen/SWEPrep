...
If M is the number of set function calls, N is 
the number of get function calls, and L 
is average length of key and value strings.
Time: O(L * M * log(M)) for get
      O(N * (L * log M + log M)) for set
Space: O(M * L) for set
       O(N) for get
'''
class TimeMap:
    ...
    def get(self, key: str, timestamp: int) -> str:
        ...
        # Do binary search and look on getting info on right
        it = self.time_map[key].bisect_right(timestamp)
